...
final class PageData
{
  ...
  static function get_shared_data()
  {
    ...
    if (file_exists($shared_file_path)) {
      return self::$shared = Yaml::parseFile($shared_file_path);
    } else {
      return array();
    }
  }
  ...
  static function parse_vars($vars, $markdown_compatible, $relative_path)
  {
    foreach ($vars as $key => $value) {
      ...
      // Replace {{ path }} placeholder with absolute path
      $absolute_path = Helpers::relative_path_to_absolute_url($relative_path);
      $value = preg_replace('/{{\s*path\s*}}/', $absolute_path . '/', $value);

      $value = trim($value);

      // Parse as markdown if needed
      if ($markdown_compatible && str_contains($value, "\n")) {
        $value = MarkdownParser::transform($value);
      }

      // Fix relative image paths to absolute paths
      // $value = self::fix_image_paths($value, $relative_path);
